...
import com.google.jribble.ast.{JribbleProtos=>P}
...
// TODO(spoon) sort this in the same order as the proto file
abstract class ProtobufConverter extends AnyRef with JribbleAnalysis {
  ...
  def globalName(sym: Symbol, addModuleSuffix: Boolean): P.GlobalName = {
    ...
    proto.setName(sym.encodedName + suffix)
   ...
   * TODO(spoon) try to move this to a narrower scope
   */
  private val labelSyms = mut.Set.empty[Symbol]
  ...
  def convertExpr(tree: Tree, enclosingClass: Symbol): P.Expr = trace(tree) {
    ...
    (tree: @unchecked) match {
      ...
      if c.tag == ClassTag =>
        ...
        classLiteral.setName(globalName(x.typeSymbol))
      ...
      case tree: Ident =>
        ...
        varRef.setName(tree.symbol.encodedName)
      ...
      case tree@Select(qualifier, name) =>
        ...
        fieldRef.setName(tree.symbol.encodedName)
  ...
  def convertCatch(catchClause: CaseDef, enclosingClass: Symbol): P.Catch = {
    ...
    proto.setBody(convertStatement(catchClause.body, enclosingClass))
    proto.build
  }
              
  // TODO(lspoon) move labelSyms to a parameter to this method
  def convertStatement(tree: Tree, enclosingClass: Symbol): P.Statement = {
    ...
    tree match {
      case Block(stats, expr) =>
        ...
        for (stat <- stats) block.addStatement(convertStatement(stat, enclosingClass))
      ...
      case tree@LabelDef(_, _, innerStatement) =>
        ...
        labelledStat.setLabel(tree.symbol.encodedName.toString)
        labelledStat.setStatement(convertStatement(innerStatement, enclosingClass))
      ...
      case tree:Apply if labelSyms.contains(tree.symbol) =>
        ...
        cont.setLabel(tree.symbol.encodedName.toString)
      ...
      case If(cond, exp1, exp2) =>
        ...
        ifStat.setThen(convertStatement(exp1, enclosingClass))
        if (!exp2.isEmpty) {
          ifStat.setElsee(convertStatement(exp2, enclosingClass))
        }
      
      case Try(block, catches, finalizer) =>
        proto.setType(P.Statement.StatementType.Try)
        val tryStat = P.Try.newBuilder
        tryStat.setBlock(convertStatement(block, enclosingClass))
        for (catchClause <- catches) tryStat.addCatch(convertCatch(catchClause, enclosingClass))
        if (!finalizer.isEmpty) tryStat.setFinalizer(convertStatement(finalizer, enclosingClass))
      ...
      case tree@Match(selector, cases) =>
        ...
        for ((constants,stat) <- switches; constant <- constants) {
          ...
          caseClause.setStatement(convertStatement(stat, enclosingClass))
          switch.addCase(caseClause)
        }
        if (default.isDefined) switch.setDefaultCase(convertStatement(default.get, enclosingClass))
  ...
  def declaration(tree: Tree, enclosingClass: Symbol): P.Declaration = trace(tree) {
    ...
    tree match {
      ...
      case tree@DefDef(mods, name, tparams, vparamss, tp, rhs) =>
        ...
        method.setBody(convertStatement(rhs, enclosingClass))
  ...
  def declaredType(clazz: ClassDef): P.DeclaredType.Builder = trace(clazz) {
    ...
    for (decl <- clazz.impl.body) proto.addBody(declaration(decl, clazz.symbol))
  ...
  def mirrorClassFor(module: Symbol): P.DeclaredType = {
    val proto = P.DeclaredType.newBuilder
    proto.setName(globalName(module))
    val mods = P.Modifiers.newBuilder
    mods.setIsFinal(true)
    mods.setIsPublic(true)
